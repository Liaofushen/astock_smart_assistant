...
def create_stock_charts(df):
    ...
    macd, signal, hist = talib.MACD(
        typical_price, fastperiod=12, slowperiod=26, signalperiod=9
    )
    ...
    fig = make_subplots(
        rows=4,
        cols=1,  # 改为4行以容纳K线、成交量、MACD和KDJ
        shared_xaxes=True,
        vertical_spacing=0.05,
        row_heights=[0.4, 0.2, 0.2, 0.2],  # 调整每个子图的高度比例
        specs=[
            [{'secondary_y': False}],  # K线图不需要副Y轴
    ...
    # 添加成交量图（在第二行）
    colors = [
        'red' if row['收盘'] >= row['开盘'] else 'green' for _, row in df.iterrows()
    ]
    fig.add_trace(
        go.Bar(
            x=df.index, y=df['成交量'], name='成交量', marker_color=colors, opacity=0.3
        ),
        row=2,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(x=df.index, y=df['D'], name='D', line=dict(color='orange')),
        row=4,
        col=1,
    )
    fig.add_trace(
        go.Scatter(x=df.index, y=df['J'], name='J', line=dict(color='purple')),
        row=4,
        col=1,
    )
